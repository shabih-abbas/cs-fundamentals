...
PNG illinify(PNG image) {
  for(unsigned x=0; x< image.width(); x++){
    for(unsigned y=0; y< image.height(); y++){
      HSLAPixel & pixel= image.getPixel(x,y);
      if(pixel.h<216){
        pixel.h=(std::abs(pixel.h-216)< std::abs(pixel.h-11))? 216 : 11;
      }
      else{
        pixel.h=(std::abs(pixel.h-216)< std::abs(pixel.h-371))? 216 : 11;
      }
    }
  }
  return image;
  ...
  for(unsigned x=0; x<firstImage.width() && x<secondImage.width(); x++){
    for(unsigned y=0; y<firstImage.height() && y<secondImage.height(); y++){
      if(secondImage.getPixel(x,y).l==1.0){
        firstImage.getPixel(x,y).l= (firstImage.getPixel(x,y).l<=0.8)? firstImage.getPixel(x,y).l + 0.2 : 1.0;
      }
    }
  }
  return firstImage;
